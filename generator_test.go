...
func TestSchemaHashOptionWithNamespace(t *testing.T) {
	schema := `
webrpc = v1
...
`
	...
	output := generateSwift(t, schema, "-schemaHash=false", "-visibility=internal", "-namespace=WaasGenerated")
	requireContains(t, output, "internal enum WaasGenerated {")
	requireContains(t, output, "Code generated by webrpc-gen@")
	requireNotContains(t, output, "WEBRPC_VERSION")
	requireNotContains(t, output, "WEBRPC_SCHEMA_VERSION")
	requireNotContains(t, output, "WEBRPC_SCHEMA_HASH")

	project := writeSwiftPackage(t, "schema-hash-namespace", map[string]string{
		"Sources/Generated/Client.swift": output,
	})
